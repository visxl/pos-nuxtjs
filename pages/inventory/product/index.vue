    ...
    <main class="grid items-start flex-1 gap-4 ">
      <Tabs default-value="product">
        <div class="flex items-center">
          <TabsList>
            <TabsTrigger value="product"><span class="text-blue-700">Product</span></TabsTrigger>
            <TabsTrigger value="quantity"><span class="text-blue-700">Quantity</span></TabsTrigger>
          </TabsList>
          ...
          <div class="flex items-center gap-2 ml-auto">
            ...
            <UButton size="sm" class="text-blue-800 bg-blue-200 disabled:opacity-50 hover:bg-blue-100 disabled:bg-blue-400"
              ...
              @click="viewProductById(selectedProducts)"
            >
            ...
            <UButton size="sm" class="text-green-700 bg-green-200 disabled:opacity-50 hover:bg-green-100 disabled:bg-green-400"
              :disabled="selectedProducts.length === 0"
              @click="editProductById(selectedProducts)"
            >
            ...
            <UButton size="sm" class="text-red-700 bg-red-200 disabled:opacity-50 hover:bg-red-100 disabled:bg-red-400"
              :disabled="selectedProducts.length === 0"
              @click="deleteProductsByIds(selectedProducts)"
            >
              <Trash class="w-4 h-4" />
              <span>Delete</span>
            </UButton>
            ...
            <div class="px-4 py-4">
              <div class="flex justify-between px-3 ">
                <div class="text-blue-700">
                  <h2>Product</h2>
                  <p class="text-blue-700 opacity-70">Here is a list of all product</p>
                </div>
                <div class="flex justify-start gap-4 py-3">
                  <div class="relative items-center w-full max-w-64">
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead class="hidden md:table-cell">Unit</TableHead>
                    <TableHead class="hidden md:table-cell">Status</TableHead>
                  </TableRow>
                </TableHeader>
                
                <TableBody v-if="product && product.length">
                  <TableRow v-for="item in product" :key="item.id" class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800"
                  ...
                  >
                    ...
                    <TableCell class="font-sans">{{ item.productName || '-' }}</TableCell>
                    <TableCell class="hidden md:table-cell">{{ item.categoryName || '-' }}</TableCell>
                    <TableCell class="hidden md:table-cell">{{ item.cost || '-' }}</TableCell>
                    <TableCell class="hidden md:table-cell">{{ item.price || '-' }}</TableCell>
                    <TableCell>
                      <UBadge class="text-center"
                        :class="[
                          'text-center',
                          item.quantity <= 5 ? 'text-red-500 bg-red-100 border-red-500' :
                          item.quantity > 0 ? 'text-green-500 bg-green-100 border-green-500' : 'text-gray-500 border-gray-300 '
                        ]"
                      >
                       {{ item.quantity || '0' }}
                      </UBadge>
                    ...
                    <TableCell class="hidden md:table-cell">{{ item.unit || '-' }}</TableCell>
                    <TableCell class="hidden md:table-cell">
                      <UBadge size="sm"
                        :class="[
                          'text-center',
                          item.status === true ? 'text-blue-500 bg-blue-100 border-blue-500' :
                          item.status === false ? 'text-red-500 border-red-500 bg-red-100' : 'text-gray-500 border-gray-300 '
                        ]"
                      >
                        {{ item.status === true ? 'Enabled' : item.status === false ? 'Disabled' : '-' }}
                      </UBadge>
                    </TableCell>
                  </TableRow>
                </TableBody>
...
<script setup lang="ts">
  import { Search, EyeIcon, PenSquareIcon, Trash } from "lucide-vue-next";
  import CreateProduct from "@/components/Modal/createProduct.vue";
  import AddQuantity from "~/components/Product/AddQuantity.vue";
